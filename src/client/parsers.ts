  ...
  parser: (_) => {
    ...
    if (content && "fields" in content) {
      ...
      const result = {
        ...
        attributes: (typeof display?.data === "object" && display?.data !== null) ?
          (content?.fields?.attributes?.fields?.map?.fields?.contents ?? {}) :
          {},
      };
...
export const LIMITED_FIXED_PRICE_MARKET_REGEX =
  // eslint-disable-next-line max-len
  /0x2::dynamic_field::Field<(0x[a-f0-9]{63,64})::limited_fixed_price::MarketKey, 0x[a-f0-9]{63,64}::limited_fixed_price::LimitedFixedPriceMarket</;
